...
function buildMenu(p) {
  ...
  const header =
    ...
    `║  🤖 *${botName}*  ║\n` +
    `╚══════════════════════╝\n` +
    `_Prefix:_ *${p}*  |  _Prefixless:_ ${pl ? "*ON* ✅" : "OFF"}`;
  return header;
}

// Full paginated menu — split into sections so no single message exceeds WhatsApp's limit
function buildMenuSections(p) {
  if (!p) p = getPrefix();

  const sec = (icon, title, items) => menuSection(icon, title, items.map(([c, d]) => [`${p}${c}`, d]));
  ...
  return [
    ...
    [
      sec("ℹ️", "General", [
        ...
        ["uptime",  "Bot uptime & RAM usage"],
        ["ping",    "Check response latency"],
        ["time",    "Current UTC date & time"],
        ["stats",   "Bot usage analytics"],
      ]),
      sec("⚙️", "Settings & Features", [
        ["botsettings",           "View all bot settings"],
        ["features",              "View all feature toggles"],
        ["feature [name] on/off", "Toggle any feature"],
        ["lang [code]",           "Set bot language (e.g. fr, es)"],
        ["langs",                 "List supported languages"],
        ["setprefix [char]",      "Change command prefix (admin)"],
        ["mode [public/private/group]", "Set bot access mode (admin)"],
      ]),
    ],
    // ── Page 2: AI & Search ────────────────────────────────────────────
    [
      sec("🤖", "AI & Smart Features", [
        ["ai [text]",        "Smart AI chat reply"],
        ["ask [question]",   "Quick question & answer"],
        ["imagine [prompt]", "Generate an AI image"],
        ["tts [text]",       "Convert text to voice note"],
        ["summarize [text]", "Summarise any long text"],
        ["clearchat",        "Clear your AI chat history"],
      ]),
      sec("🌍", "Search & Info", [
        ["weather [city]",  "Live weather report"],
        ["wiki [topic]",    "Wikipedia summary"],
        ["define [word]",   "Dictionary definition + phonetic"],
        ["tr [lang] [text]","Translate text to any language"],
        ["langs",           "List all supported translate codes"],
      ]),
    ],
    // ── Page 3: Fun ────────────────────────────────────────────────────
    [
      ...
      sec("🔠", "Text Tools", [
        ...
        ["calc [expr]",         "Safe calculator (e.g. 2^8)"],
        ["emojify [text]",      "🇪🇲🇴🇯🇮 flag-letter text"],
        ["italic [text]",       "𝘐𝘵𝘢𝘭𝘪𝘤 unicode text"],
        ["lower [text]",        "convert text to lowercase"],
        ["mock [text]",         "mOcKiNg SpOnGeBoB text"],
        ["repeat [n] [text]",   "Repeat text N times (max 20)"],
        ["reverse [text]",      "esreveR your text"],
        ["upper [text]",        "CONVERT TEXT TO UPPERCASE"],
      ]),
    ],
    // ── Page 4: Media & Utilities ──────────────────────────────────────
    [
      sec("🎬", "Media & Files", [
        ["convert",       "Convert media format (reply to file)"],
        ["dl [url]",      "Download video from URL"],
        ["music [query]", "Search & download music"],
        ["s / sticker",   "Create sticker from image/video"],
        ["v",             "Reveal view-once photo/video/audio"],
        ["yt [url]",      "Download YouTube audio"],
      ]),
      sec("🔧", "Utilities", [
        ["calc [expr]",   "Calculator with ^ power support"],
        ["pp [@user]",    "Get someone's profile picture"],
        ["qr [text/url]", "Generate a QR code image"],
        ["short [url]",   "Shorten a long URL via TinyURL"],
        ["whois [@user]", "User info card (group only)"],
      ]),
    ],
    // ── Page 5: Shopping & Booking ─────────────────────────────────────
    [
      sec("🛒", "Shopping", [
        ["shop / catalog",    "Browse the product catalog"],
        ["order [id]",        "Place an order by product ID"],
    ...
    // ── Page 6: Group Commands ─────────────────────────────────────────
    [
      sec("👥", "Group — Info & Members", [
        ["add [number]",    "Add a member to the group"],
        ["admins",          "List all group admins"],
        ["groupinfo",       "Show group information"],
        ["members / count", "Show member & admin count"],
        ["whois [@user]",   "Show user info card"],
      ]),
      sec("📢", "Group — Tagging", [
        ["everyone [msg]",  "Tag every member with a message"],
        ["hidetag [msg]",   "Silent-tag all (no visible @names)"],
        ["tagall [msg]",    "Tag all members loudly"],
      ]),
    ],
    // ── Page 7: Group Admin Actions ────────────────────────────────────
    [
      sec("🔒", "Group — Admin Actions", [
        ["close",           "Lock group — only admins can send"],
        ["del",             "Delete a message (reply to it)"],
        ["demote @user",    "Remove admin rights from member"],
        ["kick @user",      "Remove a member from the group"],
        ["mute",            "Mute the group (same as close)"],
        ["open",            "Open group — all members can send"],
        ["promote @user",   "Give admin rights to member"],
        ["unmute",          "Unmute the group (same as open)"],
      ]),
      sec("🔗", "Group — Link & Settings", [
        ["add [number]",        "Add member by phone number"],
        ["glink",               "Send group invite link to your DM"],
        ["link / invitelink",   "Get the group invite link"],
        ["revoke / resetlink",  "Reset & regenerate invite link"],
        ["setdesc [text]",      "Update the group description"],
        ["seticon",             "Set group photo (reply to image)"],
        ["setname [name]",      "Rename the group"],
      ]),
    ],
    // ── Page 8: Group Moderation ───────────────────────────────────────
    [
      sec("🛡️", "Group — Moderation", [
      ...
      sec("🔔", "Group — Auto Features", [
        ["antidelete on/off",   "Recover deleted messages"],
        ["antilink on/off",     "Delete messages containing links"],
        ["antimentiongroup on/off", "Block mass @everyone mentions"],
        ["antispam on/off",     "Rate-limit spammy members"],
        ["antitag on/off",      "Block mass member tagging"],
        ["poll Q | A | B | C",  "Create a WhatsApp poll"],
        ["setwelcome [msg]",    "Set a join welcome message"],
      ]),
    ],
    // ── Page 9: Super Admin ────────────────────────────────────────────
    [
      sec("👑", "Super Admin — Bot Control", [
        ["anticall on/off",         "Auto-reject incoming calls"],
        ["autolike on/off",         "Auto-like status updates"],
        ["autoread on/off",         "Auto-read all messages"],
        ["autorecording on/off",    "Show 🎤 recording for voice msgs"],
        ["autotyping on/off",       "Show ⌨️ typing for all messages"],
        ["autoview on/off",         "Auto-view status updates"],
        ["broadcast [msg]",         "Send message to all known users"],
        ["clearmenuvideo",          "Remove the menu video"],
        ["delkeyword [keyword]",    "Delete an auto-reply keyword"],
        ["feature [name] on/off",   "Toggle any bot feature"],
        ["keywords",                "List all auto-reply keywords"],
        ["mode [public/private/group]", "Set bot access mode"],
        ["prefixless on/off",       "Allow commands without prefix"],
        ["setkeyword [kw] | [reply]","Add an auto-reply keyword"],
        ["setmenuvideo",            "Set menu video (reply to video)"],
        ["setprefix [char]",        "Change command prefix"],
        ["typingdelay on/off",      "Add human-like typing delay"],
  ...
  try {
    switch (cmd) {
      ...
      case "menuv": {
        ...
        // ── Header card ────────────────────────────────────────────────────
        const headerCard = buildMenu(prefix);

        if (menuVideo) {
          // Send video with a short caption (WhatsApp caps video captions at ~1024 chars)
          await sock.sendMessage(from, {
            video: menuVideo,
            caption: headerCard,
            mimetype: "video/mp4",
          ...
          await reply(sock, msg, headerCard);
        }

        // ── Send each menu page as a separate message ──────────────────────
        const pages = buildMenuSections(prefix);
        for (const page of pages) {
          const pageText = page.join("\n\n");
          await sock.sendMessage(from, { text: pageText }, { quoted: msg });
        }
        break;
      }

...
function buildMenu(p) {
  ...
  const pNote = pl ? `\n_✅ Prefixless ON — commands work without \`${p}\` too_` : "";
  return `╔═══════════════════════╗
║  🤖 *${botName} MENU* 🤖  ║
╚═══════════════════════╝
...
› \`${p}ask [question]\` — Quick answers
› \`${p}imagine [prompt]\` — Generate AI image
› \`${p}tts [text]\` — Text to speech
› \`${p}summarize [text]\` — Summarize anything
...
› \`${p}langs\` — Supported languages

🎮 *Fun & Games*
› \`${p}8ball [question]\` — Magic 8-ball
› \`${p}flip\` — Coin flip
› \`${p}roll [max]\` — Roll a dice
› \`${p}joke\` — Random joke
› \`${p}fact\` — Random fun fact
› \`${p}quote\` — Inspirational quote

🔠 *Text Tools*
› \`${p}mock [text]\` — SpOnGeBoB mock
› \`${p}reverse [text]\` — Reverse text
› \`${p}aesthetic [text]\` — Ａｅｓｔｈｅｔｉｃ
› \`${p}bold [text]\` — 𝗕𝗼𝗹𝗱 𝘁𝗲𝘅𝘁
› \`${p}italic [text]\` — 𝘐𝘵𝘢𝘭𝘪𝘤 𝘵𝘦𝘹𝘵
› \`${p}emojify [text]\` — 🇪🇲🇴🇯🇮
› \`${p}upper\` / \`${p}lower\` — CASE change
› \`${p}repeat [n] [text]\` — Repeat text
...
👤 *Profile & Users*
› \`${p}pp [@user]\` — Get profile picture
› \`${p}whois [@user]\` — User info card
› \`${p}alive\` — Bot alive check
› \`${p}uptime\` — Bot status & uptime
...
› \`${p}setkeyword [kw] | [reply]\`
› \`${p}ban\` / \`${p}unban\` [number]
› \`${p}autoview\` / \`${p}autolike on/off\`
› \`${p}anticall on/off\` — Anti call
› \`${p}voreveal on/off\` — Auto view-once
